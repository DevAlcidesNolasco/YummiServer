...
export const login = async (req, res) => {
  ...
  const matchedPassword = await User.verifyPassword({ password, hash })
  if (!matchedPassword) return res.status(401).json({ message: 'Credenciales invalidas, verifique el correo o la contraseña' })
